...
namespace Siscon_API_Backend.Controllers
{
    ...
    public class ApartamentoController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] int numeroApartamento, [FromQuery] int andar, [FromServices] SisconDataContext Dbcontext)
        {
            try
            {
                ...
                {
                    apartamento = apartamento.Where(x => x.NumeroApto == numeroApartamento);
                }

                if (andar > 0)
                {
                    apartamento = apartamento.Where(x => x.Andar == andar);
                }

                var apartamentos = await apartamento.OrderBy(x => x.Id).ToListAsync();
